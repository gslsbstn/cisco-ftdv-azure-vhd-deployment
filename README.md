...
- no real Azure subscription IDs, tenant IDs or resource IDs are present
- no company-specific resource group, VNet, subnet or DNS names are present
- no public source IPs for your organization are present
- no Cisco license, FMC registration key or bootstrap secret is present
- `terraform.tfvars.example` contains only example values

You can run a quick local scan with:

```bash
rg -n "company-name|real-subscription-id|real-tenant-id|password|secret|token|registration" .
```
